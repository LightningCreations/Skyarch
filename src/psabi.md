...
Non-Trivial Aggregates are types with an alignment greater than 4, or a class type C++ with one of the following special member functions being non-trivial, and which is not _trivially relocatable_:
...
_ If the paramater/return value is larger than 8 bytes in size, it is passed/returned in memory,
_ If the parameter/return value is a Non-Trivial Aggregate, it is passed/returned in memory, \* Otherwise, it is passed directly/returned.
